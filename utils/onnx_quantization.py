...
def make_parser():
    ...
    parser.add_argument(
        "--legacy",
        help="enable legacy mode for legacy yolox models",
        action="store_true",
    )
    return parser
...
def preprocess_image(
    image_path, height, width, swap=[0, 1, 2], channels=3, legacy=False
):
...
def preprocess_func(images_folder, height, width, swap, size_limit=0, legacy=False):
    ...
    for image_name in batch_filenames:
        ...
        image_data = preprocess_image(
            image_filepath, height, width, swap=swap, legacy=legacy
        )
...
class YOLOXDataReader(CalibrationDataReader):
    ...
    def __init__(self, calibration_image_folder, input_name, input_shape, legacy=False):
...
if __name__ == "__main__":
    ...
    dr = YOLOXDataReader(
        args.cal, input_name, (image_height, image_width), legacy=args.legacy
    )
    quantize_static(
        model_input,
        model_output,
